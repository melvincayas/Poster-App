...
postSchema.post('findOneAndDelete', async function(doc) {
    if (doc) {
        ...
        await mongoose.model('User').updateOne({ posts: doc._id }, { $pull: { posts: doc._id } });
        // deletes post from PostOwner
        await mongoose.model('User').updateMany({ comments: { $in: doc.comments }}, { $pull: { comments: { $in: doc.comments } } });
